...
def list_enrollments(id: int, db: Session = Depends(get_db)):
    enrollements = (
        db.query(StudentEnrollment).filter(StudentEnrollment.student_id == id).all()
    )
    return enrollements
...
def list_enrollments(id: int, db: Session = Depends(get_db)):
    enrollements = (
        db.query(StudentEnrollment).filter(StudentEnrollment.course_id == id).all()
    )
    return enrollements
...
def list_enrollments(id: int, db: Session = Depends(get_db)):
    enrollements = (
        db.query(StudentEnrollment).filter(StudentEnrollment.branch_id == id).all()
    )
    return enrollements
...
def list_enrollments(id: int, db: Session = Depends(get_db)):
    enrollements = (
        db.query(StudentEnrollment).filter(StudentEnrollment.division_id == id).all()
    )
    return enrollements
...
def create_enrollment(enrollment: EnrollmentCreate, db: Session = Depends(get_db)):
    db_enrollment = (
        ...
        .filter(StudentEnrollment.enrollment_number == enrollment.enrollment_number)
        .first()
    )
    if db_enrollment:
        raise HTTPException(
