...
from app.schemas.attendance_records import AttendanceRecordCreate, AttendanceRecordUpdate, AttendanceRecordOut
...
@router.post("/", response_model=AttendanceRecordOut, dependencies=[Depends(require_admin)])
def create_attendance_record(record: AttendanceRecordCreate, db: Session = Depends(get_db)):
    ...
    if db_record.first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Attendance record already exists for this student in this timetable")
...
@router.put("/{record_id}", response_model=AttendanceRecordOut, dependencies=[Depends(require_admin)])
def update_attendance_record(record_id: int, record_in: AttendanceRecordUpdate, db: Session = Depends(get_db)):
...
@router.delete("/{record_id}", response_model=AttendanceRecordOut, dependencies=[Depends(require_admin)])
