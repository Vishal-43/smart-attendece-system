...
def create_access_token(data: dict) -> str:
    to_encode = data.copy()
    exp_min = datetime.utcnow() + timedelta(minutes=int(os.getenv('ACCESS_TOKEN_EXPIRE_MINUTES')))
...
def decode_token(token:str) -> dict:
    try:
        decode_token = jwt.decode(token=token,key=os.getenv('JWT_SECRET'),algorithms=[os.getenv('JWT_ALGORITHM')])
        return decode_token
